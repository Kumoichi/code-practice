...
DIによる違いは、大きく分けて**「テスト」「変更の波及範囲」「責務」**の3つの観点で整理できる。

| 3つの観点 | DIを使用しない | DIを使用する |
| --- | --- | --- |
...
| **2. 変更の波及範囲** | UseCaseが具体的な実装に依存するため、DBやキャッシュなどの変更がApplication層やテストにまで波及しやすい | インターフェースを介するため、実装を変更してもUseCaseやテストへの影響を抑えやすい |
| **3. 責務** | UseCaseがDBの生成や接続など、インフラの詳細まで意識しやすい | オブジェクトの組み立てを外部に任せられるため、UseCaseは「何をするか」というビジネスロジックに集中しやすい |
...
ここで見たいのは「差し替えられて便利」ではなく、**依存先の都合が変わったとき、書き換える範囲がどこまで広がるか**です。DBの乗り換えは滅多に起きませんが、依存先の都合が変わること自体は機能追加のたびに起きます。

その波及範囲を測る道具として、「キャッシュを追加したい」というお題を使います。素の`BoxRepository`をキャッシュで包む`CachedBoxRepository`を用意するところまでは3つとも共通で、違いは**それを実際に使わせるために、どこまで書き換える羽目になったか**です。

実際にgitのコミットとして分けてあります。いったん3つすべてからキャッシュを取り除いた状態を作り、そこから同じ機能をディレクトリごとに1コミットずつ追加し直しているので、**各コミットの差分がそのまま「その設計で同じ機能を足すのに何を書き換える羽目になるか」の答え**になっています。
...
| 1-no-di | （そもそも無い） | 型1箇所＋処理2行 | （元々テスト不能） | ccde1f3 |
...
「キャッシュに差し替えられて嬉しい」ではありません。**ビジネスロジックが、どれだけインフラの都合に縛られているか**です。

キャッシュを挟むかどうかは、本来`persistence`層だけの都合のはずです。ところが`2-di-concrete`と`1-no-di`では、その都合が`application`層の型定義や、そのテストコードにまで漏れ出しました。`3-di-interface`では`persistence`層の中で完結しています。
