...
ここまでの`3-di-interface`では、依存の組み立てを[main.go](go/clean-architecture/repository-interface/3-di-interface/main.go)に手で書いていました。
...
Goの実務でDIというと、この「組み立て」を自動化する[Wire](https://github.com/google/wire)（Google製）の名前がよく出てきます。実際に`3-di-interface`をWire版にしたものを[4-di-wire](go/clean-architecture/repository-interface/4-di-wire/)に用意したので、それを見ながら説明します。
...
|  | 何をするものか | どのディレクトリか |
| --- | --- | --- |
| DI（設計） | 依存を引数で受け取る | 3-di-interface / 4-di-wire 共通 |
| Wire（道具） | 依存の組み立てコードを自動生成する | 4-di-wireのみ |

`domain`・`application`・`persistence`は、`3-di-interface`と`4-di-wire`でimportパス以外まったく同じです。Wireを入れてもUseCaseやRepositoryのコードは変わらず、変わるのは組み立ての書き方だけです。
...
人間が書くのは[wire.go](go/clean-architecture/repository-interface/4-di-wire/wire.go)です。
...
`wire`コマンドを実行すると、[wire_gen.go](go/clean-architecture/repository-interface/4-di-wire/wire_gen.go)が生成されます（抜粋）。
...
生成されるのは、`3-di-interface`の`main.go`に手で書いていた組み立てと同じ内容の、**ただのGoコード**です。実行時に何かが動的に解決されるわけではなく、コンパイル前に生成されたコードがそのまま使われます。そのため、依存が足りなければ`wire`コマンドの時点でエラーになり、実行時のリフレクションによる遅さも、DIコンテナ特有の「動かして初めて気づく」問題もありません。これがWireが選ばれる主な理由です。
...
正直に言うと、このリポジトリの規模（依存が`db` → `repository` → `useCase`の3つ）では、Wireは**過剰**です。`4-di-wire`は`3-di-interface`より、`wire.go`・`wire_gen.go`・`wire`コマンドの実行という手間の分だけ、むしろ増えています。

Wireが効いてくるのは次のようなときです。

- 依存が数十個あり、`main.go`の組み立てが長く、順番管理が負担になっている
- コンストラクタの引数が頻繁に変わり、そのたびに組み立てコードを直すのが面倒
- 組み立て漏れをコンパイル前に機械的に検出したい
